...
REM "C:\Program Files\CGpython\python.exe" %scanner% -begin -p=YourComponent -v=%FULL_VERSION% -b=%branch% -d=%BASE_DIR% -t=dotcover -r=%XUNIT_RESULTS_PATH%\dotCover.html -x=exclusions
...
REM See Dev Guide for additional detail: https://confluence.capgroup.com/display/SLM/Python+Developer+Guide
REM set BUILD_UTIL=%CG_TOOLS_HOME%\c\interface_scripts\buildutil
REM C:\Program Files\CGpython\python.exe %BUILD_UTIL%\python\build.py <SOME_FOLDER>\requirements.txt
REM if %ERRORLEVEL% NEQ 0 goto fail

REM Add for Powershell Module Build
REM echo Bootstrap powershell dependencies
REM PowerShell.exe -Command ". E:\synced_repos\interface_scripts\buildutil\powershell\PSModuleCI.ps1; BootStrapDependencies"

REM echo Compress into zip package
REM cd source
REM dir
REM "%CG_TOOLS_HOME%\c\win32\7Zip\7za.exe" a -tzip %CG_BUILD_PKG_ID%.zip *.* -r
